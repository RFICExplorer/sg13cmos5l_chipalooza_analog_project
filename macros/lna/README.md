...
<p align="center">
  <a href="render/img/lna_white.png">
    <img src="render/img/lna_white.png" alt="Render of the ihp-sg13cmos5l lna layout" width=70%>
  </a>
  <br>
  <em>Render of the ihp-sg13cmos5l lna layout.</em>
</p>

This is the analog example **sub-macro** of the Chipalooza analog project template: the unit `lna` cell with its complete flow (schematic → simulation → layout → DRC/LVS/PEX → post-layout simulation → characterization). The hand-drawn top level that embeds it, including the chip-level power straps and the PR boundary, lives one directory up in [`sg13cmos5l_chipalooza_analog_project`](../../README.md).
...
│  ├─ 📁 pex/
│  │  ├─ lna_klayout_pex_*.spice
│  │  └─ lna_magic_pex_*.spice
│  └─ 📁 schematic/
...
All targets that operate on a specific cell accept an optional `CELL=<cellname>` parameter. The default is the top-level cell (`lna`).
...
```sh
make sim-xschem                     # run the default testbench (lna_tb_tran)
...
```sh
make sim-xschem TB=lna_tb_ac_ol
make sim-xschem TB=lna_tb_tran
make sim-xschem TB=lna_tb_dc_vout
```
...
```sh
make sim-view-xschem                      # run the default plotting script (plot_lna)
...
```sh
make sim-view-xschem SCRIPT=plot_lna
```


### CACE Simulations

Runs [CACE](https://github.com/fossi-foundation/cace) characterization for the lna macro using `verification/cace/lna.yaml`. CACE is part of the IIC-OSIC-TOOLS container.
...
For each run, selected result plots are copied to `verification/cace/results/lna/`, and temporary `_runs` folders are cleaned between runs. At the end, `_runs`, `_docs`, and `netlist` under `verification/cace/` are removed.
...
- `verification/cace/results/lna/`
  - `Adc_ol_dB_mm.png`, `fcu_mm.png`
  - `Adc_ol_dB_mc.png`, `fcu_mc.png`
  - `Adc_ol_dB_vs_vdd.png`, `fcu_vs_vdd.png`

> [!NOTE]
> The result callback in `verification/cace/scripts/lna_tb_ac.py` must not `print()`. CACE 2.11.0 redirects stdout into its rich logger while the script runs and the logger writes back to stdout, so any print from that hook recurses without end.
...
> [!NOTE]
> Unlike the [top level](../../README.md#pr-boundary-check), this macro has no `check-boundary` step. The PR boundary box on layer 189 is only needed by the cell the chip flow places, which is `sg13cmos5l_chipalooza_analog_project`, not the `lna` cells inside it.
...
Renders the top-level layout GDS with `sak-render.py` from the [IIC-OSIC-TOOLS](https://github.com/iic-jku/IIC-OSIC-TOOLS) and saves the two images `lna_black.png` and `lna_white.png` (2048 px wide, 4x oversampling) in `render/img/`:
...
```sh
make klayout-drc
make klayout-drc CELL=lna
make klayout-drc CELL=lna DRC_LEVEL=regular
```
...
make klayout-lvs-netlist CELL=lna
...
make magic-lvs-netlist CELL=lna
...
```sh
make symbol-pex                  # build lna_pex.sym from lna.sym
...
```sh
python3 scripts/check_pex_ports.py netlist/pex/lna_magic_pex_2.spice
...
```sh
make klayout-pex
make klayout-pex CELL=lna
make klayout-pex CELL=lna EXT_MODE=3
```
...
```sh
make magic-pex
make magic-pex CELL=lna
make magic-pex CELL=lna EXT_MODE=3
```
...
```sh
make magic-pex CELL=lna EXT_MODE=3 THRESHOLD=5000 MINRES=500 MINDELAY=2
```
...
```sh
make klayout-verify
make klayout-verify CELL=lna
```

**Verify all cells** (currently only `lna`):
...
6. Rename the CACE files `verification/cace/lna.yaml`, `verification/cace/templates/lna_tb_ac.sch` and `verification/cace/scripts/lna_tb_ac.{py,csv}`, and set `name:` in the yaml.
7. Rename the plotting scripts in `testbenches/xschem/plot_simulations/`. The sizing notebook `scripts/sizing/sizing_lna.ipynb` and the figures next to it are specific to the lna, so adapt or delete them.
8. Search and replace the remaining `lna` references inside the renamed files. Xschem schematics, the CACE yaml and the plot scripts are all plain text. The ones that matter are the `lna.sym` instances in the testbenches, the `.include` of the PEX netlist, the `template:` and `script:` keys in the CACE yaml, and the raw file names in the plot scripts.
...
```sh
cp -r macros/lna macros/amp
cd macros/amp
make clean
# set TOP = amp in the Makefile, then:
for f in schematic/xschem/lna* testbenches/xschem/lna* layout/lna* \
         verification/cace/lna* verification/cace/templates/lna* \
         verification/cace/scripts/lna* \
         testbenches/xschem/plot_simulations/plot_lna*; do
    mv "$f" "$(echo "$f" | sed 's/lna/amp/')"
done
```
